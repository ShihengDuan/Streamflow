...
def load_data(station): ## Station in String
    ...
    x = np.concatenate((precip, tmax), axis=1)
    return x, y
...
for station in stations:
    ...
    best_nse = 0; model_name='DCNN/'+str(station)+'_365_pt_DCNN.h5' ## Save the best nse and best model. 
    for training_id in range(15):
        ensemble_name='DCNN/'+str(station)+'_355_ps_'+str(training_id)+'.h5'
...
np.save('DCNN/d_dcnn_365_pt_nse', d_nse)
np.save('DCNN/d_dcnn_365_pt_r', d_r)
np.save('DCNN/d_dcnn_365_pt_mae', d_mae)
np.save('DCNN/d_dcnn_365_pt_mse', d_mse)
