...
def build_model():
    model = Sequential()
    model.add(keras.layers.CuDNNGRU(50, return_sequences=True))
    model.add(keras.layers.CuDNNGRU(50, return_sequences=True))
    model.add(keras.layers.CuDNNGRU(50, return_sequences=False))
...
for station in stations:
    ...
    best_nse = -100; model_name='GRU/'+str(station)+'_GRU.h5' ## Save the best nse and best model. 
    ...
    for training_id in range(15):
        ensemble_name = 'GRU/'+str(station)+'_GRU_'+str(training_id)+'.h5'
        model = build_model()
...
np.save('GRU_NSE', d_nse);
np.save('GRU_MSE', d_mse);
np.save('GRU_high_mse', high_mse)
np.save('GRU_low_mse', low_mse)
print('done')
