...
def build_model(W,L):
    x_in = keras.layers.Input(shape=(W,1)) # Batch, Length, Dimension
    ## Block 1
...
for station in stations:
    ...
    best_nse = 0; model_name='DCNN/'+str(station)+'_365_p_DCNN.h5' ## Save the best nse and best model. 
    for training_id in range(15):
        ensemble_name='DCNN/'+str(station)+'_365_p_'+str(training_id)+'.h5'
        ...
        del model
        del adam
    print(station,': run time is ', total_time/10, 's')
    print(best_nse)
...
np.save('DCNN/d_dcnn_365_p_nse', d_nse)
np.save('DCNN/d_dcnn_365_p_r', d_r)
np.save('DCNN/d_dcnn_365_p_mae', d_mae)
np.save('DCNN/d_dcnn_365_p_mse', d_mse)
